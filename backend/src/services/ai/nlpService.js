...
const PRIORITY_KEYWORDS = {
  ...
  MEDIUM: ["media", "normal", "moderado", "medium", "normal", "moderate"],
  LOW: ["baja", "minor", "cuando puedas", "low", "minor", "whenever"],
};
...
function extractPriority(text) {
  ...
  return {
    ...
    matched: null,
    reason:
      "No se detectó prioridad específica, usando prioridad media por defecto",
  };
}
...
function extractDate(text) {
  ...
  // Formato: "DD de MONTH" (permite "el 25 de octubre" o "25 octubre")
  const dayMonthRegex = /(?:\bel\s+)?(\d{1,2})\s+(?:de\s+)?([a-záéíóúñü]+)/gi;
  while ((match = dayMonthRegex.exec(lowerText)) !== null) {
    const day = parseInt(match[1], 10);
    const monthName = match[2].toLowerCase();
    const monthNum = MONTHS[monthName];

    if (!Number.isNaN(day) && monthNum !== undefined) {
      const date = new Date();
  ...
  // Formato: "MONTH DD" (e.g., "October 25")
  const monthDayRegex = /([a-záéíóúñü]+)\s+(\d{1,2})/gi;
  while ((match = monthDayRegex.exec(lowerText)) !== null) {
    ...
    if (!Number.isNaN(day) && monthNum !== undefined) {
      ...
      return { date, confidence: 0.85, type: "absolute", matched: match[0] };
    }
  }
...
function extractAssignee(text) {
  ...
  for (const keyword of ASSIGNMENT_KEYWORDS) {
    ...
    if (index !== -1) {
      ...
      const candidateSegment = afterKeyword
        .split(/[,.;]|\b(?:para|for|due|antes|before|by|hasta|el)\b/i)[0]
        .trim();

      // Extraer el primer nombre (considerando nombre compuesto)
      const nameMatch = candidateSegment.match(
        /^([A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÑüñ]+(?:\s+[A-ZÁÉÍÓÚÑ][A-Za-zÁÉÍÓÚÑüñ]+)?)/
      );
      if (nameMatch) {
        return {
...
function extractTitleAndDescription(text) {
  ...
  Object.values(PRIORITY_KEYWORDS)
    .flat()
    .forEach((keyword) => {
...
export function parseTaskInstruction(instruction) {
  ...
  } = extractTitleAndDescription(trimmedInstruction);
...
export function calculateNLPMetrics(samples) {
  ...
  samples.forEach((sample) => {
    ...
    if (expected.dueDate && actual.dueDate) {
      const diffDays = Math.abs(
        (new Date(actual.dueDate) - new Date(expected.dueDate)) /
          (1000 * 60 * 60 * 24)
      );
    ...
    if (expected.assignee && actual.assignee) {
      if (
        actual.assignee
          .toLowerCase()
          .includes(expected.assignee.toLowerCase()) ||
  ...
  return {
    totalSamples,
    overallAccuracy:
      (correctPriority + correctDate + correctAssignee + correctTitle) /
      (totalSamples * 4),
    fieldAccuracy: {
